...
interface  fields{ name: string; label: string; type: string ,options?:string[]}
interface formVersionsType {
  buttonText: string;
  title: string;
  fields:fields[];
}
...
const formVersions: formVersionsType[] = [
  {
    title: "Get in Touch",
    fields: [
...
export default function ContactForm({ version }: ContactFormProps) {
  ...
  const handleChange = (
    ...
    if (type === "checkbox") {
      ...
      setFormData((prevState) => {
        const existingValue = prevState[name];
        const newValue = checkbox.checked
          ? [...(Array.isArray(existingValue) ? existingValue : []), value]
          : (Array.isArray(existingValue) ? existingValue : []).filter(
              (item) => item !== value
            );
  ...
  return (
    <section className="contact-form">{renderContactFormVersion()}</section>
  );
}
